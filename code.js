...
// Supabase project (replacing the Google Sheet above as the datastore).
// SUPABASE_PUBLISHABLE_KEY is safe to commit as plain text: it's the
// public/client-safe key, meaningless without the RLS policies defined in
// the project's schema — it is NOT the service_role key. If a server-side
// operation ever needs to bypass RLS, store that key in Script Properties
// (Project Settings > Script Properties in the Apps Script editor) and
// read it with PropertiesService.getScriptProperties().getProperty(...)
// instead of adding it here.
const SUPABASE_URL = 'https://fcloggepmjmkbwkemtsy.supabase.co';
const SUPABASE_PUBLISHABLE_KEY = 'sb_publishable_bWyPGkTqS_deKX408f1RDg_YttmYLpG';
